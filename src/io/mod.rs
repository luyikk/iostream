...
pub trait Stream{
    ...
    ///  ms.copy_to(&mut fs);
    /// ```
    fn copy_to(&mut self, target:&mut Stream) -> Result<(), String> {

        if !self.can_read(){
            return Err("the current stream cannot read".to_string())
        }

        if !target.can_write(){
            return Err("the target stream cannot read".to_string())
        }
    ...
    fn set_length(&mut self,u64)->Result<(),String>;
